...
- Master -> semisync-slave -> async-slave:
  - Состоит из одного мастера и двух реплик. Одна реплика полусинхронная, вторая - асинхронная;
  - Количество занимаемого места умножается на 3.
...
365 days * 202 TB/day * 3 / 100 TB = 2 221;
```
THROUGHPUT фотографии (физические файлы):
```
Throughput = MAX DAU/sec = 59,25 GB/sec;

Использование RAID - массива для повышения пропускной способности на чтение (вопрос на QA как правильно посчитать RAID-массив).
Также вопрос по пропускной способности сети - как правильно посчитать или не учитывать сеть в расчетах?
```
